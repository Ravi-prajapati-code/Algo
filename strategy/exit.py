...
Exit hierarchy (checked in order, first trigger wins):
  1. ATR hard stop    — max(STOP_LOSS_PCT%, 1.5 × ATR) below entry
  2. Take profit      — 13 % above entry (configurable)
  3. ATR trailing stop— 1.5 × ATR below peak price (tighter than fixed %)
  4. Trend reversal   — EMA20 death cross
  5. RSI overbought   — RSI ≥ 75 (take profit on extended move)
  6. Time-based exit  — held > MAX_HOLD_DAYS calendar days
  7. RS deterioration — stock starts underperforming index while in drawdown

ATR-based stops adapt to each stock's own volatility:
  - Volatile stock (ATR 3 %) → stop placed 4.5 % below entry
  - Calm stock (ATR 1 %)     → stop placed max(2 %, 1.5 %) = 2 % below
...
def check_exit(
    ...
    # ── 5. RSI overbought — extended move ─────────────────────────────────
    rsi = ind.get("rsi", 0)
    if rsi >= RSI_OVERBOUGHT_EXIT:
        return True, f"SIGNAL: RSI overbought ({rsi:.1f} ≥ {RSI_OVERBOUGHT_EXIT})"

    # ── 6. RS deterioration (exit if underperforming AND in drawdown) ────────
